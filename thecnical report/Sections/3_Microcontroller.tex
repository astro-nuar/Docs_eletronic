...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Images/3_Microcontroller/USB_coms.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Images/3_Microcontroller/current_setup.jpeg}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Images/3_Microcontroller/STM32411.jpeg}
...
\subsection{Stages of flight}

In Figure~\ref{fig.3:State_diagram_visual} it is possible to see the state that the rocket will report during flight while in Figure~\ref{fig.3:State_diagram_computer} it is possible to see the actual state diagram that the internal SRAD computer will take into account considering that it is the only states that system actuation/sensing changes.
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images/3_Microcontroller/State_diagram_visual.drawio}
        \caption{Visual}
        \label{fig.3:State_diagram_visual}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{Images/3_Microcontroller/State_diagram_computer.drawio.pdf}
        \caption{Computer}
        \label{fig.3:State_diagram_computer}
    \end{subfigure}

    \caption{SRAD state diagrams}
    \label{fig.3:State_diagrams}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{BOOT}
    \begin{itemize}
        \item Establish communication with the ground station.
        \item Verify sensor status, internal variables, and subsystem health.
        \item Perform SRAD computer self-checks.
        \item Await ``GO'' confirmation from both the SRAD computer and the ground station team.
    \end{itemize}

    \item \textbf{BOOT $\rightarrow$ ARMED}
    \begin{itemize}
        \item Triggered by an arming command sent from the ground station to the SRAD computer.
    \end{itemize}

    \item \textbf{ARMED}
    \begin{itemize}
        \item Start SD card flight-data logging.
        \item Continuously monitor telemetry and sensor streams.
    \end{itemize}

    \item \textbf{ARMED $\rightarrow$ LAUNCH}
    \begin{itemize}
        \item Triggered after launch authorization and motor ignition detection.
    \end{itemize}

    \item \textbf{LAUNCH}
    \begin{itemize}
        \item Initialize the Kalman filter.
        \item Fuse accelerometer, gyroscope, and magnetometer measurements to estimate:
        \begin{itemize}
            \item Acceleration
            \item Velocity
            \item Position
    ...
    \item \textbf{LAUNCH $\rightarrow$ BOOST}
    \begin{itemize}
        \item Triggered when longitudinal acceleration exceeds $3g$.
    \end{itemize}

    \item \textbf{BOOST}
    \begin{itemize}
        \item Monitor powered ascent dynamics.
        \item Continue inertial state estimation using IMU data.
    \end{itemize}

    \item \textbf{BOOST $\rightarrow$ COAST}
    \begin{itemize}
        \item Triggered when vertical acceleration drops below $0$, indicating motor burnout.
    \end{itemize}

    \item \textbf{COAST}
    \begin{itemize}
        \item Continue estimating altitude and vertical velocity.
        \item Prepare recovery systems for apogee deployment.
    \end{itemize}

    \item \textbf{COAST $\rightarrow$ APOGEE}
    \begin{itemize}
        \item Triggered when estimated vertical velocity becomes negative.
    \end{itemize}

    \item \textbf{APOGEE}
    \begin{itemize}
        \item Integrate GPS measurements into the navigation filter for improved position tracking.
        \item Deploy the drogue parachute to stabilize descent.
    \end{itemize}

    \item \textbf{APOGEE $\rightarrow$ MAIN CHUTE}
    \begin{itemize}
        \item Triggered when altitude falls below the predefined deployment threshold according to EUROC regulations of 450m (needs to be adjusted to a more reasonably value that is not exactly at the maximal allowed).
    \end{itemize}

    \item \textbf{MAIN CHUTE}
    \begin{itemize}
        \item Deploy the main parachute.
        \item Monitor descent rate and landing conditions.
    \end{itemize}

    \item \textbf{MAIN CHUTE $\rightarrow$ LANDED}
    \begin{itemize}
        \item Triggered when vertical velocity remains within a near-zero threshold for more than $5$ seconds. This condition reduces false landing detections caused by wind disturbances or transient motion.
    \end{itemize}

    \item \textbf{LANDED}
    \begin{itemize}
        \item Stop flight operations.
        \item Finalize data logging and preserve recovery telemetry.
        \item Await recovery team retrieval.
    \end{itemize}
...
\subsubsection{task data acquisition}
